...
def webscrape_data(ticker, company):
    ...
    this_filter = df.filter(['cost-goods-sold', 
                ...
                'net-income', 'ebitda', 'ebitda-margin',
...
def get_dataset():
    ...
    df['ebit'] = df['ebit'].replace('', 0) 
    df['cash-flow-from-operating-activities'] = df['cash-flow-from-operating-activities'].replace('', 0) 
    df['current-ratio'] = df['current-ratio'].replace('', 0) 
    df['debt-equity-ratio'] = df['debt-equity-ratio'].replace('', 0) 
    df['gross-margin'] = df['gross-margin'].replace('', 0) 
    df['net-profit-margin'] = df['net-profit-margin'].replace('', 0) 
    df['roe'] = df['roe'].replace('', 0) 
    df['past-eps-earnings-per-share-diluted'] = df['past-eps-earnings-per-share-diluted'].replace('', 0) 
    df['eps-earnings-per-share-diluted'] = df['eps-earnings-per-share-diluted'].replace('', 0) 
        
    df['revenue'] = df['revenue'].fillna(0)
    df['ebit'] = df['ebit'].fillna(0)   
    df['cash-flow-from-operating-activities'] = df['cash-flow-from-operating-activities'].fillna(0)
    df['current-ratio'] = df['current-ratio'].fillna(0)
    df['debt-equity-ratio'] = df['debt-equity-ratio'].fillna(0)
    df['gross-margin'] = df['gross-margin'].fillna(0)
    df['net-profit-margin'] = df['net-profit-margin'].fillna(0)
    df['roe'] = df['roe'].fillna(0)
    df['past-eps-earnings-per-share-diluted'] = df['past-eps-earnings-per-share-diluted'].fillna(0)
    df['eps-earnings-per-share-diluted'] = df['eps-earnings-per-share-diluted'].fillna(0)
    # cols = df.columns
